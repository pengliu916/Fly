...
D3DUtil_CameraKeys CBaseCamera::MapKey( UINT nKey )
{
    ...
    switch( nKey )
    {
       /* case VK_CONTROL:
        ...
        //case VK_PRIOR:
        //    return      CAM_MOVE_BACKWARD;// pgup
        //case VK_NEXT:
        //    return      CAM_MOVE_FORWARD; // pgdn
...
VOID CFirstPersonCamera::FrameMove( FLOAT fElapsedTime )
{
	...
	mainTran=vPosDeltaWorld;
	if(m_vEye.x>2.0f||m_vEye.x<-2.0f)
	{
		m_vEye.x-=vPosDeltaWorld.x;
		mainTran.x*=0;
	}
	if(m_vEye.z>2.0f||m_vEye.z<-2.0f)
	{
		m_vEye.z-=vPosDeltaWorld.z;
		mainTran.z*=0;
	}
